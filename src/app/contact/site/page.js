                ...
                {sites.map((place, index) => {
                    return (
                        <div key={index} className={`w-full flex flex-col md:flex-row bg-[#31363F] bg-opacity-70 rounded-2xl shadow-md shadow-black items-center lg:items-start ${index % 2 != 0 && "md:flex-row-reverse"}`}>
                            ...
                            <div className={`flex items-center justify-center w-full lg:w-1/3 bg-cover bg-center p-40 rounded-2xl ${index % 2 != 0 ? "md:rounded-l-none" : "md:rounded-r-none"}`} style={{ backgroundImage: `url('/images/sites/${place.image}');` }}></div>
