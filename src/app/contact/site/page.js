                ...
                {sites.map((place, index) => {
                    return (
                        <div key={index} className={`w-full flex flex-col md:flex-row bg-[#31363F] bg-opacity-70 rounded-2xl shadow-md shadow-black items-center lg:items-start ${index % 2 != 0 && "md:flex-row-reverse"}`}>
                            <Image src={`/images/sites/${place.image}`} width={1000} height={1000} className={`object-cover object-center w-full md:w-1/3 min-w-[100px] md:min-w-[300px] rounded-2xl ${index % 2 != 0 ? "md:rounded-l-none" : "md:rounded-r-none"}`} alt="place to stay in" />
