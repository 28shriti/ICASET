...
const BoardMember = (props) => {
    return (
        <Link href={props["link"] !=undefined ? props.link : "#"} className={`h-full w-[385px] text-white ${props.className}`} target={props["link"] != undefined ? "_blank" : "_self"}>
            <div className="bg-[#31363F] bg-opacity-70 flex items-center gap-5 rounded-2xl hover:scale-[1.05] p-5 h-full w-full shadow-md shadow-black" style={{transition: 'all 0.4s'}}>
                <Image src={props.img} height={1000} width={1000} alt="img" className="rounded-full w-[100px] h-[100px] object-center object-cover" />
                <div>
